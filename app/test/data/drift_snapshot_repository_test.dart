...
void main() {
  ...
  test('keeps zero valued counters', () async {
    await repository.save(
      snapshotAt(DateTime(2026, 1, 15), const {'Seer Points': 0}),
    );
  ...
  test('every snapshot gets a distinct identifier', () async {
    final a = await repository.save(
      snapshotAt(DateTime(2026, 1, 1), const {'Hacks': 1}),
    );
    final b = await repository.save(
      snapshotAt(DateTime(2026, 1, 2), const {'Hacks': 2}),
    );
  ...
  test('sorts most recent first, whatever the insertion order', () async {
    await repository.save(
      snapshotAt(DateTime(2026, 1, 10), const {'Hacks': 10}),
    );
    await repository.save(
      snapshotAt(DateTime(2026, 1, 30), const {'Hacks': 30}),
    );
    await repository.save(
      snapshotAt(DateTime(2026, 1, 20), const {'Hacks': 20}),
    );
  ...
  test('deleting a snapshot takes its counters with it', () async {
    final stored = await repository.save(
      snapshotAt(DateTime(2026, 1, 15), const {'Hacks': 1}),
    );
  ...
  test('two snapshots do not mix their counters', () async {
    await repository.save(
      snapshotAt(DateTime(2026, 1, 1), const {'Hacks': 10}),
    );
    await repository.save(
